...
class PO_Composer {
	...
	function admin_menu_item() {
		$menu_page = add_management_page(__('PO Composer', 'themerex'), __('PO Composer', 'themerex'), 'manage_options', 'po_composer', array($this, 'build_page'));
	}
	...
	function load_styles() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			wp_enqueue_style('po-composer-style', get_template_directory_uri() . '/admin/po_composer/po_composer.css', array(), null);
		}
	}
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			...
			wp_enqueue_script('po-composer-script', get_template_directory_uri() . '/admin/po_composer/po_composer.js', array('jquery'), null, true);
		}
	}
		...
		if (!empty($this->po_src)) {
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					file_put_contents($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					file_put_contents($mo_temp, $this->mo_text);
	...
	function loadPO($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.$suffix] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.$suffix]['tmp_name']) && file_exists($_FILES['po_file'.$suffix]['tmp_name']) ? file($_FILES['po_file'.$suffix]['tmp_name']) : '';
				if (empty($rez['data'])) {
			...
			// or load file from 'languages' folder
				...
				$name = escapeshellcmd(substr($_POST['po_src'.$suffix], strpos($_POST['po_src'.$suffix], '_')+1));
				$rez['data'] = file_exists($dir.'/'.$name.'.po') ? file($dir.'/'.$name.'.po') : '';
				if (empty($rez['data'])) {
