	...
	if($sku != '') 
	{
		...
		$skucount = count($skus);
			...
			$i = 0; foreach($skus as $sku) {
				...
				/** Check if there is 1 or more products, and open products wrapper **/
				$shortcode .= ($i % $cols == 0 && $skucount >= 2) ? '<div class="products">' : ''; 
				
				$product_data = Mage::getModel('catalog/product')
				->getCollection()
				->addAttributeToSelect(array('name', 'product_url', 'thumbnail', 'price', 'special_price', 'group_price', 'short_description', 'preorder'))
				...
				if($product_data->getName() != "") { // If product exists
					...
					$shortcode .= '<div class="product'.$singleclass.'">';
						...
						$shortcode .= '<a class="product_img" href="'.$poductUrl.'" title="' . $product_data->getName() . '">';
								...
								$shortcode .= '<img title="' . $product_data->getName() . '" src="' . Mage::helper('catalog/image')->init($product_data, 'thumbnail')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize($img_width) . '" alt="' . $product_data->getName() . '">';
						$shortcode .= '</a>';
						}
						...
						if($price == "true") 
						{	
							$shortcode .= preg_replace( '/\s+/', ' ', $theProductBlock->getPriceHtml($product_data, true));					
						}
						...
						{						
							if($product_data->getPreorder()){
							      $buttonTitle = Mage::helper('core')->__('Pre-Order');
							  }else{
							      $buttonTitle = Mage::helper('core')->__('Add to Cart');
							  }
							if($btn_link == 'button') 
							{
								$shortcode .= '<button class="form-button ' . $button_color . '" onclick="setLocation(\''. Mage::helper('checkout/cart')->getAddUrl($product_data) .'\')"><span>'. $buttonTitle .'</span></button>';
							} 
							
							else 
							{
								$shortcode .= '<a class="' . $button_color . '" href="'.Mage::helper('checkout/cart')->getAddUrl($product_data).'" title="'.$name.'">'. $buttonTitle .'</a>';
							}
						}
