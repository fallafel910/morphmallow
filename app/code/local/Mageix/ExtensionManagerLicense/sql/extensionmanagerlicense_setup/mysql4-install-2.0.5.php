...
CREATE TABLE IF NOT EXISTS `{$this->getTable('extensionmanagerlicense_license')}` (
...
try {
    $this->run("INSERT INTO `{$this->getTable('mextensionmanager_module')}` (module_name, download_uri)
        VALUES ('Mageix_ExtensionManagerLicense', 'https://ixcba.com/ixval/index/memupdate')");
} catch (Exception $e) {
    // already exists
}
