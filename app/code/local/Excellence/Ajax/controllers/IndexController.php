...
class Excellence_Ajax_IndexController extends Mage_Checkout_CartController
{
		...
		if($params['isAjax'] == 1){
			...
			try {
				...
				if (!$cart->getQuote()->getHasError()){
					$message = $this->__('<span><i class="fa fa-check"></i></span><p><strong>%s</strong> was added to your shopping cart.</p>', Mage::helper('core')->htmlEscape($product->getName()));
	...
	public function updateItemOptionsAction()
	{
		...
		try {
			...
			if (!$cart->getQuote()->getHasError()){
			    $response['status'] = 'SUCCESS';
			    $response['message'] = $this->__('<span><i class="fa fa-check"></i></span><p><strong>%s</strong> was updated in your shopping cart.</p>', Mage::helper('core')->htmlEscape($item->getProduct()->getName()));
			    $this->loadLayout();
	...
	public function deleteAction(){
        ...
        if ($id) {
			...
			$product = $this->_initProduct();
			$message = $this->__('<span><i class="fa fa-check"></i></span><p><strong>item</strong> was deleted from your shopping cart.</p>');
