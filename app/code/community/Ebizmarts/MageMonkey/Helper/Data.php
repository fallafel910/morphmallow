...
class Ebizmarts_MageMonkey_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	{
        Mage::log('tut', null, '$isConfirmNeed.log');
		...
		if( !empty($curlists) ){
            Mage::log('tut 1', null, '$isConfirmNeed.log');
		...
		if(is_array($lists) && is_array($curlists)){
            Mage::log('tut 2', null, '$isConfirmNeed.log');
            Mage::log(Mage::getStoreConfig(Mage_Newsletter_Model_Subscriber::XML_PATH_CONFIRMATION_FLAG, Mage::app()->getStore()->getId()), null, '$isConfirmNeed.log');

            Mage::log($lists, null, '$isConfirmNeed.log');
            Mage::log($curlists, null, '$isConfirmNeed.log');

			$subscribe = array_diff_key($lists, $curlists);
			if( !empty($subscribe) ){
                Mage::log('tut 3', null, '$isConfirmNeed.log');
				foreach($subscribe as $listId => $slist){
                    Mage::log('tut 4', null, '$isConfirmNeed.log');
					if(!isset($slist['subscribed'])){
						continue;
					}
                    Mage::log('tut 5', null, '$isConfirmNeed.log');
					$groupings = $lists[$listId];
					unset($groupings['subscribed']);
                    ...
                    Mage::log($isConfirmNeed, null, '$isConfirmNeed.log');
					if($defaultList == $listId){
                        ...
                        Mage::log('tut 6', null, '$isConfirmNeed.log');
                        Mage::log($mergeVars, null, '$isConfirmNeed.log');
